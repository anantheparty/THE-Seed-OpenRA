# THE-Seed Web Console 设计文档

## 概述

Web 端控制台，用于远程操控 OpenRA 游戏和 AI Agent。

## 访问方式

- **公网**: `https://openra.ananthe.party` (nginx 反代 + Basic Auth)

---

## 页面布局 (已确认 ✅)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│ [服务控制栏]                                                          顶部  │
│ [Git Pull] [编译] [启动游戏] [重启] [停止] | 状态: ● 运行中                 │
├───────────────────────────────────────────┬─────────────────────────────────┤
│                                           │                                 │
│                                           │  [对话区] - Tab 切换            │
│           VNC 游戏画面                    │                                 │
│           (可交互)                        │  ┌─────────────────────────┐    │
│                                           │  │ [副官] [敌方]           │    │
│           宽度: 60%                       │  ├─────────────────────────┤    │
│           高度: 固定 720px                │  │                         │    │
│                                           │  │ 聊天历史                │    │
│                                           │  │ (可滚动)                │    │
│                                           │  │                         │    │
│                                           │  ├─────────────────────────┤    │
│                                           │  │ [输入框] [发送]         │    │
│                                           │  └─────────────────────────┘    │
│                                           │                                 │
│                                           │  宽度: 40%                      │
├───────────────────────────────────────────┴─────────────────────────────────┤
│ [Debug 工具区] - 默认折叠 ▼                                          下方   │
│                                                                             │
│ ┌─ Tabs (可扩展) ───────────────────────────────────────────────────────┐   │
│ │ [📋 Logs] [📊 Status] [🖥️ Terminal] [+]                              │   │
│ └───────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│ ┌─ 内容区 (可滚动，高度可拖拽调整) ──────────────────────────────────────┐   │
│ │                                                                       │   │
│ │ (折叠时隐藏此区域)                                                    │   │
│ │                                                                       │   │
│ └───────────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 模块设计

### 1. 服务控制栏 (顶部) ✅

| 按钮 | 命令 | 说明 |
|------|------|------|
| Git Pull | `git pull` | 拉取最新代码 |
| 编译 | `dotnet build` | 在 OpenCodeAlert 目录执行 |
| 启动游戏 | `./start.sh` | 在 OpenCodeAlert 目录执行 |
| 重启 | kill + start | 重启游戏 |
| 停止 | `pkill` | 停止游戏 |
| 启动 AI | `python main.py` | 启动 THE-Seed |

**路径**:
- OpenRA: `~/theseed/THE-Seed-OpenRA/OpenCodeAlert/`
- THE-Seed: `~/theseed/THE-Seed-OpenRA/`

---

### 2. VNC 游戏画面 (左侧 60%) ✅

- 嵌入 noVNC iframe
- 分辨率: 1280x720
- 鼠标/键盘完全映射

---

### 3. 对话区 (右侧 40%) ✅

**Tab 切换**:
- **副官 (Copilot)**: 指挥己方 AI
  - WebSocket 连接 Dashboard Bridge (ws://host:8080)
  - 发送指令、显示回复、历史记录
  
- **敌方 (Enemy)**: 预留 UI
  - 暂不实现功能
  - 保留界面和输入框

---

### 4. Debug 工具区 (下方) ✅

**默认折叠**，点击展开

**Tabs (保持可扩展)**:
| Tab | 说明 | 优先级 |
|-----|------|--------|
| 📋 Logs | THE-Seed 日志 | MVP |
| 📊 Status | 基本状态信息 | MVP |
| 🖥️ Terminal | xterm.js 终端 | MVP |
| + | 扩展入口 | 预留 |

**日志路径**: `~/theseed/THE-Seed-OpenRA/the-seed/Logs/` (后续确认)

---

## nginx 反代配置

```nginx
server {
    server_name openra.ananthe.party;

    # Basic Auth
    auth_basic "OpenRA Console";
    auth_basic_user_file /etc/nginx/.htpasswd;

    # Web Console 主页面
    location / {
        proxy_pass http://127.0.0.1:8000;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    }

    # noVNC WebSocket
    location /vnc/ {
        proxy_pass http://127.0.0.1:6080/;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host $host;
        proxy_read_timeout 86400;
    }

    # Dashboard Bridge WebSocket
    location /api/ {
        proxy_pass http://127.0.0.1:8080/;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host $host;
        proxy_read_timeout 86400;
    }

    # SSL (Certbot 会自动添加)
    listen 443 ssl;
    ssl_certificate /etc/letsencrypt/live/openra.ananthe.party/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/openra.ananthe.party/privkey.pem;
    include /etc/letsencrypt/options-ssl-nginx.conf;
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem;
}

server {
    if ($host = openra.ananthe.party) {
        return 301 https://$host$request_uri;
    }
    server_name openra.ananthe.party;
    listen 80;
    return 404;
}
```

---

## 端口规划

| 端口 | 服务 | 说明 |
|------|------|------|
| 8000 | Web Console | 静态页面服务 |
| 6080 | noVNC | VNC WebSocket |
| 8080 | Dashboard Bridge | THE-Seed WebSocket |
| 5900 | VNC Server | x11vnc (内部) |
| :99 | Xvfb | 虚拟显示 (内部) |

---

## 实现计划

### Phase 1 - MVP
- [ ] 基础布局 (VNC 60% + 对话区 40%)
- [ ] 服务控制栏 (Git Pull / 编译 / 启动 / 停止)
- [ ] 副官对话功能
- [ ] Debug 区折叠框架
- [ ] nginx 反代 + Basic Auth + SSL

### Phase 2 - 完善
- [ ] xterm.js 终端
- [ ] Logs Tab
- [ ] 敌方对话 UI (预留)

### Phase 3 - 扩展
- [ ] 更多 Debug 工具
- [ ] 性能优化

---

## 文件结构

```
web-console/
├── DESIGN.md           # 本文档
├── index.html          # 主页面
├── css/
│   └── style.css       # 样式
├── js/
│   ├── app.js          # 主逻辑
│   ├── vnc.js          # VNC 控制
│   ├── chat.js         # 对话功能
│   ├── service.js      # 服务控制
│   └── terminal.js     # xterm.js
├── start-vnc.sh        # VNC 启动脚本
└── api/
    └── service.py      # 服务控制后端 (可选)
```
