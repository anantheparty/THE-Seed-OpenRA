...
## [2026-04-05] DONE — T-R6-7: map_query字段筛选+bitpack压缩

**C#端** (228ede8 in submodule): MapQueryCommand支持params.fields JArray字段筛选；新增IsExplored_packed/IsVisible_packed bitpack（128×128 bool→512 ints）；始终返回explored_pct
**Python端** (e332c01): game_api._unpack_bool_grid()解包bitpack；map_query(fields=...)透传；WorldModel首次全量获取后续只取["IsExplored_packed","MapWidth","MapHeight"]；MapQueryResult加explored_pct；修复所有test MockWorldSource.fetch_map签名
**子模块指针** (63b1559): 更新OpenCodeAlert引用

## [2026-04-05] DONE — SYSTEM_PROMPT v2 + LLM失败日志分类

用户提出6条prompt改进建议，全部实现：
1. **开放任务里程碑**：SYSTEM_PROMPT加分阶段完成条件（矿场+矿车/兵营车厂/雷达）
2. **观测-验证规则**：success但facts不变→query_world验证→暂停同类动作
3. **动作追踪**：指示LLM回顾最近命令和预期效果
4. **warning收紧**：缺前置/暂时blocked→info，warning仅限真正危险
5. **LLM失败日志分类**（代码改动）：_classify_llm_error区分timeout/provider_error/parse_error/network_error/empty_output；slog含error_type；auto_terminate消息含错误详情
6. **query_world使用条件**：初始context已有结构化信息时不默认query

## [2026-04-05] DONE — E2E R7 深度审计 + P0修复

**审计报告**: docs/wang/r7_audit.md (36任务, 35K记录, 4个并行分析代理)

**关键发现**: session后半段(10:37起)完全瘫痪，根因是world_model/core.py一行typo `a.location`→`a.position`。65个ERROR, 22个任务死亡, 13个defend_base全crash, 基地被打爆。LLM本身252/252全部成功，但wake_cycle crash被错误归因为"LLM连续失败"。

**P0修复 (Wang)**:
- B1: `a.location`→`a.position` (world_model/core.py:631,652)
- B2: wake_cycle crash正确归因，不再标为"LLM失败"
- B3: EconomyExpert阵营判断修复 — 苏联单位在苏联玩家上不再误判为"阵营不匹配"
- B4: 新增tech_center_count, repair_facility_count到runtime_facts

**分派给Xi (T-R7-1~7)**: defend_base去重、reply(null)fallback、NLU低置信度gate、composite拆分修复、other_active_tasks加job摘要、探索算法fallback、scout_map job复用

## [2026-04-05] DONE — T-R7-1~7 全部完成

**T-R7-1** (fc4e19b): defend_base 60s cooldown — _ensure_defend_base_task 跟踪 _defend_base_last_created 时间��，即使前一个任务已failed也不在60s内重建
**T-R7-2** (9299e1b): reply(null) fallback to command — _handle_reply 返回"没有待回答的问题"时自动走 _handle_command
**T-R7-5** (accb8c2): other_active_tasks 含 job 摘要 — _other_active_tasks_for 增加 expert_type/unit_type/count/region；context 渲染为 `[并行] 建基地(running) [EconomyExpert:powrx2]`
**T-R7-3** (64c2a7d): NLU hard_min_confidence=0.7 — predict confidence 低于0.7一律 fallback 到 LLM routing
**T-R7-4** (64c2a7d): 同一 commit — produce 多 item 时每个 step 用独立 source_text（"造4个车间"而非完整原文）
**T-R7-6** (b814093): 探索算法三改进：(1)fallback扫描8x8块找最密未探索区 (2)ray角度限制在search_region象限内 (3)enemy_half动态推断（base对角线方向）
**T-R7-7** (8aadce7): SYSTEM_PROMPT加Job复用规则 — running的ReconExpert→patch_job，同unit的EconomyExpert→等待

168/168 tests passed

## [2026-04-05] DONE — T-R8-1~5 全部完成

**T-R8-1+3** (176cf2e): 新增"info"分类类型（玩家情报/反馈），_handle_info创建任务并确认。_handle_command增加语义重叠检测（≥2关键词重叠→不重复创建）
**T-R8-2** (59fd10e): SYSTEM_PROMPT加query_world重复限制——连续3次空结果后停止同参数查询
**T-R8-4** (18a2699): QueueManager只对Building/Defense队列做auto_place，跳过Vehicle/Infantry（harv不需要放置）
**T-R8-5** (295b94b): ws_server启动时检查voice子系统可用性（dashscope包+API key），缺失时记录warning

156/156 tests passed

## [2026-04-05] DONE — T-EX-0~8 Expert层优化

**T-EX-0** (d58f459): context_to_message恢复[CONTEXT UPDATE] JSON header，修复test_e2e_t10；同时修复test_e2e_t1 extract_context
**T-EX-1+2** (31a0a0f): 去掉ti*0.35角度漂移（完全破坏search_region约束）；grid cache TTL 5s→2s
**T-EX-3** (58e39d2): frontier-cell探索算法——扫描explored/unexplored边界，按8x8块聚类，方向约束+密度/距离评分。优先于random-ray
**T-EX-4** (69e1c8f): combat集火——_engage_assault改为全军集中攻击最低HP敌人，不再分散火力
**T-EX-5** (24a6c5f): idle harvester检测——track矿车位置，连续3 tick不动且非采矿状态→deploy重新采矿
**T-EX-6** (69e1c8f): 同commit——_choose_threat_direction fallback改用base对角位置替代地图中心
**T-EX-7** (139413e): MovementJob stuck检测——连续10 tick移动<2 cell→RISK_ALERT+FAILED
**T-EX-8**: 调查完毕，query_planner工具和Planner类均存在且可正常导入，无需清理

343/343 tests passed（排除4个pre-existing e2e/voice failures）

## [2026-04-06] DONE — design_v_next.md 目标架构文档

输出 `docs/wang/design_v_next.md` — 正式冻结目标架构。

五层结构：
1. Adjutant / NLU Front Door（语言入口 + 路由）
2. Commander（唯一高层战略脑）
3. Capability Managers（EconomyCapability → ReconCapability → CombatCapability）
4. Execution Experts / Jobs（自主 tick 执行）
5. Information Plane（WorldModel + RuntimeFacts + InfoExperts + Knowledge + Logs）

关键定义：
- Task 不再默认等于独立 LLM 脑（Direct / Capability / Managed 三种形态）
- 渐进收敛：Phase A（Economy Capability）→ Phase B（Commander + 更多 Capability）→ Phase C（架构收口）
- 废弃/过渡/目标 清单明确
- 与旧文档（design.md / architecture_crisis.md / capability_task_design.md）关系说明

基于 Yu 审计 wang_system_architecture_audit_20260406.md 建议 + 6篇近年 LLM+RTS 论文共同结论。
