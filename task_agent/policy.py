...
_CAPABILITY_TOOL_GUIDANCE = {
    "produce_units": "真正下单生产/建造时使用；只对当前最小里程碑或明确请求动作下单。若玩家一次明确点名多个当前可立即下单、且彼此无依赖的目标，可在同一 wake 发多个 produce_units；这种显式清单本身就是当前最小闭环，不要人为拆成多轮，但仍要避免同 queue 重复/冲突下单。",
...
- `produce_units_then_attack`
  - `request_units_first` / `waiting_for_units`：只能 request_units、wait、必要时 send_task_message / query_world；不要先启动 CombatExpert
  - `ready_to_attack` / `attack_running`：开始或推进攻击；不要回头补经济/建筑前置
...
## 观测-验证规则
如果你的请求或动作收到success/fulfilled信号，但连续2次context中runtime_facts关键数值未变化：
1. 用query_world验证建筑清单/在线状态
2. 确认不一致 → 暂停同类动作，send_task_message(type='info', content='状态不一致，暂停扩张验证中')
3. 禁止重复补同类建筑直到验证通过

## 世界同步保护
如果 [世界同步] 显示 stale=true 或连续失败上升，说明当前世界状态可能陈旧。
此时不要新开同类 job、不要基于旧状态连续重试；优先 wait，必要时 send_task_message(type='info', content='世界状态同步异常，暂停动作等待恢复')。
...
- 普通 managed task 不拥有 EconomyExpert 生产Job；看到生产/建造已在外部处理中时，发送 info 或等待，不要再补 request_units / 补链
...
3. [持续目标]不为空 → 在没有新指令时，继续朝该目标推进一个最小步骤，直到当前目标明显完成、被阻塞，或被新的玩家指令覆盖

**只有在 [待处理请求] 为空、[玩家追加指令] 为"无"、且 [持续目标] 也为空时，才必须输出"wait"。**
如果 [阶段] 已经明确显示当前推进点，只有在**已经存在请求或本轮玩家指令**时才按该推进点收敛；它本身不是开工授权。如果 [阻塞] 不为空，先解除阻塞，解除不了就 wait。

## 你不应该做的
- **没有[待处理请求]、[玩家追加指令]为"无"且[持续目标]为空时，不要主动造兵或造建筑**
- 不要因为 [基地推进] / [可立即下单] / [前置已满足] 就自行开工；这些是规划与真值，不是授权
...
- 对这种“显式多目标清单”，整条清单就是当前最小里程碑，不要把它错误拆成“先造一个，下一轮再造下一个”
- 上述并发只适用于“显式多目标且当前都可直接下单”的情况；宽泛目标（如“发展经济”“爆兵”）仍按一个最小里程碑推进，不要自己扩成并发流水线
- 例：玩家说“电厂、兵营、5个步兵”，若 `powr`、`barr`、`e1` 此刻都在[可立即下单]里，允许同一回复连续输出三个 `produce_units`
...
## 决策参考
- [可立即下单] 是唯一直接动作真值；要下单时，优先只看这里
- [前置已满足但当前受阻] 表示前置链已通，但此刻仍不能下单；先按原因解除阻塞，不要硬下单
- [前置已满足] 只是低优先级参考；如果同一项已经出现在[可立即下单]、[前置已满足但当前受阻]或[基地推进]里，忽略它在[前置已满足]中的存在，不要因此重复下单或越级补链
...
- [持续目标] 是 Capability 当前仍需收敛的玩家目标；只要它还明显未完成，就可以在后续 wake 继续推进，不要因为 [玩家追加指令] 已回到"无"就立刻停下
- ⚡低电力不是独立开工理由；只有在已有请求或本轮玩家指令要求推进时，才把恢复供电作为解除阻塞步骤
