## Current
Testing reform / signal quality: layered backend gate, direct-entry startup smoke, E2E classification guardrails, pytest-backed `__main__` runners, live-runner diagnostics consumption, deploy short-circuit truth hardening, live task-surface requirements, shared triage detail, and coordinator/base-readiness projection parity are now in place. The current focus remains the diagnostics/capability yellow zone, but the next slices should stay narrow: keep turning runtime/task/debug surfaces into directly readable “what is blocked, by what, for whom” answers, and avoid reopening already-closed runner/deploy lines.
