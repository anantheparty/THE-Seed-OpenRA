            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                    ...
                    try:
                        ...
                        if response.get("data") is None and isinstance(response.get("response"), dict):
                            response["data"] = response.get("response")
                        elif response.get("data") is None:
                            logger.debug(f"Response data is None. command={command} keys={list(response.keys())}")
                        ...
                        if data and "actors" in data and data["actors"]:
                        ...
                        if data and "frozenActors" in data and data["frozenActors"]:
