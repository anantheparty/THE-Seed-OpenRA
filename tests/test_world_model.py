...
def test_new_self_idle_actor_emits_unit_idle_event() -> None:
    frame_before = Frame(
        self_actors=[],
        enemy_actors=[],
        economy=PlayerBaseInfo(Cash=2500, Resources=300, Power=80, PowerDrained=40, PowerProvided=100),
    ...
    frame_after = Frame(
        self_actors=[
            Actor(actor_id=9, type="重坦", faction="自己", position=Location(24, 20), hppercent=100, activity="Idle"),
        ],
        enemy_actors=[],
        economy=PlayerBaseInfo(Cash=2500, Resources=300, Power=80, PowerDrained=40, PowerProvided=100),
        map_info=make_map(explored=0.5, visible=0.25),
    ...
    source = MockWorldSource([frame_before, frame_after])
    world = WorldModel(source)
    world.refresh(now=100.0, force=True)
    ...
    idle_events = [event for event in events if event.type == EventType.UNIT_IDLE]
    assert len(idle_events) == 1
    assert idle_events[0].actor_id == 9
    assert idle_events[0].data["owner"] == "self"
    print("  PASS: new_self_idle_actor_emits_unit_idle_event")
