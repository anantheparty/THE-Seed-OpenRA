...
from models import CombatJobConfig, Task, TaskMessage, TaskMessageType, TaskStatus
from models.enums import EngagementMode, MoveMode
from runtime_views import CapabilityStatusSnapshot, RuntimeStateSnapshot
...
from tests._adjutant_fixtures import (
...
def test_mock_adjutant_fixtures_match_minimum_protocol_surface():
    ...
    assert_object_surface(
        kernel,
        required_callables=KERNEL_REQUIRED_CALLABLES,
        required_attrs=KERNEL_REQUIRED_ATTRS,
        label="MockKernel",
    )
    assert_object_surface(
        world_model,
        required_callables=WORLD_MODEL_REQUIRED_CALLABLES,
        label="MockWorldModel",
    )

    runtime_state = kernel.runtime_state()
    assert_mapping_superset(runtime_state, RUNTIME_STATE_KEYS, label="MockKernel.runtime_state")
...
def test_runtime_nlu_bare_build_sequence_starts_direct_jobs_and_notifies_capability_when_available():
    ...
    async def run():
        result = await adjutant.handle_player_input("电厂，兵营")
        assert result["type"] == "command"
        assert result["ok"] is True
        assert result["routing"] == "nlu"
        assert "task_id" in result
    ...
    first_task_id = kernel._tasks[1].task_id
    adjutant.notify_task_completed(
        label=first_task_id, raw_text="电厂", result="succeeded", summary="done", task_id=first_task_id
    )
    assert len(kernel.created_tasks) == 2
    assert kernel.started_jobs[1]["config"].unit_type == "barr"
    assert len(kernel.capability_notes) == 2
...
def test_info_routes_to_best_active_task_without_creating_new_task():
    ...
    assert int(world_model.query_counts.get("battlefield_snapshot", 0) or 0) >= 1
